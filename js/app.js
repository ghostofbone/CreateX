(() => {
    ...
    let keys = [ "Present", "March 2019", "November 2018", "July 2015", "August 2010", "February 2007", "May 2004", "October 2001", "June 2000" ];
    function initSliders() {
        ...
        if (document.querySelector(".history-about__slider")) new swiper_core_Swiper(".history-about__slider", {
            modules: [ Navigation, Pagination, Parallax, Autoplay ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            ...
            navigation: {
                prevEl: ".history-about__button-prev",
                nextEl: ".history-about__button-next"
            },
            breakpoints: {
                640: {
                    slidesPerView: 1,
    ...
    function updateActiveClass() {
        ...
        menuItems.forEach((function(menuItem) {
            ...
            if (".." + currentPath === menuItemPath) menuItem.classList.add("active"); else menuItem.classList.remove("active");
        }));
    }
    window.onload = updateActiveClass;
    window.onpopstate = updateActiveClass;
    const blogItems = document.querySelector(".works-projects__cards");
    let data;
    let startItem = 0;
    let endItem = 9;
    if (blogItems) loadBlogItems();
    async function loadBlogItems() {
        const response = await fetch("files/works/works-card.json", {});
        if (response.ok) {
    ...
    function buildBlogItem(item) {
        ...
        blogItemTemplate += `<div data - id="${item.id}" class="works-projects__card card-projects">`;
        item.image ? blogItemTemplate += `<a href="${item.url}" class="card-projects__image">\n\t\t\t<img src="${item.image}" alt="Image">\n\t\t</a>` : null;
        blogItemTemplate += `<div class="card-projects__content">`;
        blogItemTemplate += `<h4 class="card-projects__title">\n\t\t\t<a href="${item.url}" class="card-projects__link">${item.title}</a>\n\t\t</h4>`;
        item.text ? blogItemTemplate += `<div class="card-projects__text text">\n\t\t\t${item.text}\n\t\t</div>` : null;
        blogItemTemplate += `<a href="${item.url}" class="card-projects__button button button--outline button--red">\n\t\t\t${item.button}\n\t\t</a>`;
    ...
    function viewMore() {
        ...
        const currentItems = document.querySelectorAll(".card-projects").length;
        const vewMore = document.querySelector(".works-projects__navigation");
        vewMore.hidden = true;
        currentItems < dataItemsLength ? vewMore.hidden = false : vewMore.hidden = true;
    }
    function documentActions(e) {
        const targetElement = e.target;
        if (targetElement.closest(".works-projects__view-more")) {
            startItem = document.querySelectorAll(".card-projects").length;
            endItem = startItem + 3;
            initBlog(data, startItem, endItem);
            e.preventDefault();
        }
